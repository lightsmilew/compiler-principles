...
词法分析器（lexer，也叫 scanner）是编译器的第一站。它把 ToyC 源程序的**字符流**切成一个个有意义的**词法单元（Token）**，并记录每个 Token 的类别、字面量和所在位置。
...
词法分析的理论基础是**有限状态自动机**。每种 Token 用一种正则规则描述，整体流程分三步：
...
2. **NFA → DFA**：加入一个新的总起点，用 ε 边连接所有规则起点，给接受状态标记 Token 类型和优先级；再用**子集构造**（ε-closure + move）合并成一个 DFA。
...
`skipWhitespaceAndComment` 必须**同时更新行列位置**：遇到换行时 `line += 1` 并把 `column` 复位为 1，普通字符则递增 `column`。块注释若到文件末尾仍未闭合，应报告**注释起始位置**，而不是只报告 EOF。
...
Token 流每行一个 Token，格式为 `<类别, 行号, 列号, "字面量">`：
...
| 用例 | 期望结果 | 考察点 |
| --- | --- | --- |
| `int main` | `KwInt` 而非 `ID` | 关键字与 ID 的区分 |
| `123` / `0` | `NUMBER` | 整数识别 |
| `<=`、`>=`、`==`、`!=`、`&&` 等 | 双字符运算符 | 最长匹配 |
...
| `/* 未闭合` | 报告注释起始位置 | 错误恢复 |
| `@`、`$` 等非法字符 | 报错后继续扫描 | 错误恢复 |
| 空文件、末尾无换行 | 正常输出 `EOF` | 边界条件 |

## 六、提交物

第一部分必须提交可以在 Linux 下**独立编译运行**的完整文件，而不是片段或伪代码。程序应从标准输入或命令行文件读取 ToyC 源程序，并按规定格式输出 Token 流。
